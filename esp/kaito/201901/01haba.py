...
def modeFR(neo,color,startNum,endNum):
    global frcount
    global frflag
    global flowothers
    global flownums
...
def modeP(neo,color,startNum,endNum):
    for x in range(MAXLEN):
    ...
    with open(filename,'r') as file:
        for l in file:#1gyou zutu miru
            ...
            if pattern == "n":
                ...
                modeFL(neo,color,startNum,endNum)
            elif pattern == "f" and dir =="l":
                modeFR(neo,color,startNum,endNum)
            elif pattern == "p":
                modeP(neo,color,startNum,endNum)
            else:
                pass
...
CSVread('nukarehaba.csv')
blacken()

CDS()

CSVread('dozohaba.csv')
blacken()

CDS()

CSVread('shasenhaba.csv')
blacken()

CDS()

CSVread('iretehaba.csv')
blacken()


CDS()

CSVread('singohaba.csv')
blacken()
